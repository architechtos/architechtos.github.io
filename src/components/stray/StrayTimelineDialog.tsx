...
const StrayTimelineDialog = ({ 
  ...
  useEffect(() => {
    if (isOpen && strayId) {
      fetchStrayAndActions();
    }
  }, [isOpen, strayId]);

  const fetchStrayAndActions = async () => {
    try {
      setIsLoading(true);
      ...
      console.error('Error fetching stray and actions:', err);
      setStray(null);
      setActions([]);
      setUserProfiles({});
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Ιστορικό Ενεργειών: {stray?.name || 'Φόρτωση...'}</DialogTitle>
        </DialogHeader>
